...
```
layer0/
  checker.py      run_layer0(name) → {verdict: PASS|SUSPECT|BLOCK, findings: [...]}
  registry.py     npm registry + downloads API (get_package_info, get_downloads)
  typosquat.py    levenshtein() + check_typosquat() vs top_packages.txt (~240 pkgs)
  age_check.py    age < 7 days + download spike ratio (5x threshold)
  maintainer.py   compares first-version vs latest-version maintainer set
  namespace.py    unscoped name vs top_scoped_packages.txt (~80 scoped pkgs)
  data/top_packages.txt        — refreshable; add entries to extend coverage
  data/top_scoped_packages.txt — refreshable; add entries to extend coverage

run_layer0.py  CLI: python run_layer0.py [--json] [--no-color] <pkg> [<pkg>...]
